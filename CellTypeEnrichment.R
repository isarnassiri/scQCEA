...
}else{output_dir = paste0(INPUT_PATH, opt$project_id, "/Inputs/10X-gex/")}
...
if(organism == "hsapiens"){
  GTF = fread(paste0(BACK_END_DATA, 'ensembl_human.txt'), stringsAsFactors = F, header = T)
  GTF = as.data.frame(GTF)
}

if(organism == "mmusculus"){
  GTF = fread(paste0(BACK_END_DATA, 'ensembl_moueSymID_humanSym.txt'), stringsAsFactors = F, header = T)
  GTF = as.data.frame(GTF)
...
if(length(grep('grouped', input.dir))==1) # catch integer(0)
{
  tSNE_Cellranger = fread(paste0(INPUT_PATH,project.id,'/10X-gex-grouped/',gex.library.id,'/outs/analysis/tsne/gene_expression_2_components/projection.csv'), stringsAsFactors = F)
  
}else{
  tSNE_Cellranger = fread(paste0(INPUT_PATH,project.id,'/10X-gex/',gex.library.id,'/outs/analysis/tsne/gene_expression_2_components/projection.csv'), stringsAsFactors = F)
}
...
if(length(grep('grouped', input.dir))==1) # catch integer(0)
{
  UMAP_Cellranger = fread(paste0(INPUT_PATH,project.id,'/10X-gex-grouped/',gex.library.id,'/outs/analysis/umap/gene_expression_2_components/projection.csv'), stringsAsFactors = F)
  
}else{
  UMAP_Cellranger = fread(paste0(INPUT_PATH,project.id,'/10X-gex/',gex.library.id,'/outs/analysis/umap/gene_expression_2_components/projection.csv'), stringsAsFactors = F)
}
